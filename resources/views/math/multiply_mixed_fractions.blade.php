...
@section('content')
    ...
    <script>
        ...
        // Function to multiply mixed fractions and simplify result
        function multiplyMixedFractions(whole1, numerator1, denominator1, whole2, numerator2, denominator2) {
            // Convert mixed fractions to improper fractions
            const improper1 = whole1 * denominator1 + numerator1;
            const improper2 = whole2 * denominator2 + numerator2;

            // Multiply the numerators and denominators separately
            const productNumerator = improper1 * improper2;
            const productDenominator = denominator1 * denominator2;

            // Calculate the whole number, numerator, and denominator
            const productWhole = Math.floor(productNumerator / productDenominator);
            const productNumeratorRemainder = productNumerator % productDenominator;

            return {
                whole: productWhole,
                numerator: productNumeratorRemainder,
                denominator: productDenominator
            };
        }

        // Calculate multiplication on button click
        document.getElementById('calculate-button').addEventListener('click', function() {
            ...
            // Multiply each subsequent mixed fraction with the initial one
            for (let i = 1; i < wholes.length; i++) {
                const whole = parseInt(wholes[i].value);
                const numerator = parseInt(numerators[i].value);
                const denominator = parseInt(denominators[i].value);
                const result = multiplyMixedFractions(
                ...
                resultFraction.whole = result.whole + Math.floor(result.numerator / result.denominator);
